...
<!-- Site: 'Feuchtigkeit' -->
       ...
       <body id="reportsPage">
        <div class="" id="home">
            <div class="container">
                ...
                <!-- 1. white block, includes LineChart 'Luftfeuchtigkeit'-->
                <div class="row tm-content-row tm-mt-big">
                    <div class="tm-col tm-col-big">
                        <div class="bg-white tm-block h-100">
                        <h2 class="tm-block-title">Luftfeuchtigkeit Diagramm</h2>
                            <!-- LineChart 'Luftfeuchtigkeit' -->
                            <canvas id="lineChartFeuchte" height="300"></canvas>
                        </div>
                    </div>
                    
                    <!-- 2. white block, includes DataTable 'Luftfeuchtigkeit'-->
                    <div class="tm-col tm-col-big">
                        <div class="bg-white tm-block h-100">
                        <h2 class="tm-block-title">Luftfeuchtigkeit Tabelle</h2>
                        <!-- DataTable 'Luftfeuchtigkeit' -->
                        <table id="dataTableFeuchte">
                            <thead>
                            <tr>
                                <th>Zeit</th>
                                <th>Luftfeuchte %</th>
	...
	<script src="js/actionTerraFungaTitle.js"></script>
        ...
        <!-- js to draw LineChart 'Luftfeuchtigkeit' -->
        <script>
		   ...
		   options: {
              plugins: {
                  zoom: {
                    ...
                    zoom: {
                        ...
                        threshold: 2,
                        sensitivity: 3,
                        onZoom: function({chart}) { console.log(`Im zooming!!!`); },
			  ...
			  scales: {
				 ...
				 yAxes: [{
					...
					scaleLabel: {
					   display: true,
					   labelString: "relative Luftfeuchtigkeit in %"
					}
				 }]
			  }
		   }
		...
		fetch( "dht_data.php" )
			 ...
			 .then( json => {
				  ...
				  config.data.datasets[0].data = json.map( row => row.Luftfeuchtigkeit ) ;
				  config.data.datasets[1].data = json.map( row => row.Mindestfeuchtigkeit ) ;                  
				  console.table( config.data.datasets[0].data ) ;
                  
                  // create Chart
				  var ctx   = document.getElementById( "lineChartFeuchte" ).getContext( "2d" ) ;
				  var chart = new Chart( ctx, config ) ;
			 } )
			 .catch( error => alert("Error: "+error) ) ;
		</script>
        
        <!-- js to draw DataTable 'Luftfeuchtigkeit' -->
        <script>
            $('#dataTableFeuchte').DataTable( {
            "searching": false,
            "bLengthChange": false,
            "pageLength": 20,
            "scrollY": "300px",
